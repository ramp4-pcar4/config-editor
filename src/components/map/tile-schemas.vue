...
import type { RampTileSchemaConfig } from '@/definitions';
...
const addTileSchema = () => {
  tileSchemas.push({ id: '', name: '', extentSetId: '', lodSetId: '' });
  thumbnailTileUrls.push('');
};

const removeTileSchema = (idx: number) => {
  tileSchemas?.splice(idx, 1);
  thumbnailTileUrls.splice(idx, 1);
};

const emit = defineEmits(['update:modelValue']);

const tileSchemas = reactive<Array<RampTileSchemaConfig>>(props.modelValue ?? []);
let thumbnailTileUrls = reactive<Array<string>>([]);

onBeforeMount(() => {
  thumbnailTileUrls = props.modelValue?.map((ts) => ts.thumbnailTileUrls?.join(', ') ?? '') ?? [];
});

const onThumbnailTileUrlsInput = (e: Event, index: number) => {
  thumbnailTileUrls[index] = (e.target as HTMLInputElement).value;
  tileSchemas[index].thumbnailTileUrls =
    thumbnailTileUrls[index] === ''
      ? []
      : thumbnailTileUrls[index].split(',').map((url) => url.trim());
};
  ...
  <Collapsible :thick-border="true">
    <template #header>
      ...
      >
        ...
        <span class="px-2"> Add tile schema </span>
      </button>
    </template>
    <template #default>
      <draggable v-if="tileSchemas.length > 0" :list="tileSchemas" item-key="fake" handle=".handle">
          ...
          <Collapsible>
            <template #header>
              ...
              <span class="mr-1 sm:mr-5 sm:text-lg">{{
                element.id || `Tile Schema ${index + 1}`
              }}</span>
              <button @click.stop="removeTileSchema(index)" class="ml-auto">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
              ...
              <div class="input-table">
                ...
                <div>
                  <InputHeader
                    ...
                    required
                  />
                  <input type="text" v-model="element.extentSetId" />
                </div>
                <div>
                  <InputHeader
                    title="Lod Set ID"
                    ...
                    required
                  />
                  <input type="text" v-model="element.lodSetId" />
                </div>
                <div>
                  <InputHeader
                    title="Thumbnail Tile URLs"
                    description="The comma separated list of two horizontally-adjacent tile URLs used to create the basemap thumbnail. URLs take the form of '/tile/<LOD_ID>/<VERTICAL_TILE_INDEX>/<HORIZONTAL_TILE_INDEX>'."
                  />
                  <input
                    type="text"
                    :value="thumbnailTileUrls[index]"
                    @input="(e: Event) => onThumbnailTileUrlsInput(e, index)"
                  />
                </div>
              </div>
              <div class="flex items-center mt-4">
                <input type="checkbox" v-model="element.hasNorthPole" />
                <InputHeader
