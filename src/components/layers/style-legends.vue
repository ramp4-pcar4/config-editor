...
const addStyleLegend = () => {
  styleLegends.push({ name: '', url: '' });
};

const removeStyleLegend = (idx: number) => {
  styleLegends?.splice(idx, 1);
};
...
<template>
  <Collapsible>
    <template #header>
      ...
      <span class="text-lg">Style Legends ({{ styleLegends.length }})</span>
      <!-- add item button -->
      <button
        @click.stop="addStyleLegend"
      ...
      >
        ...
        <span class="px-2"> Add style legend </span>
      </button>
    </template>
    <template #default>
      <div v-if="styleLegends.length > 0" class="style-legends-grid">
        <div></div>
        <InputHeader
        ...
        <template #item="{ element, index }">
          <div>
            ...
            <div class="style-legends-grid">
              ...
              <div class="flex justify-center">
                <input type="text" class="w-full max-w-xs" v-model="element.url" />
              </div>
              <button @click="removeStyleLegend(index)">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
