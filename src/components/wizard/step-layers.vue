<template>
    <div>
        ...
        <p class="mt-4 text-sm text-gray-600">{{ t('wizard.layers.description') }}</p>
        ...
        <div class="mt-16 rounded-xl border border-gray-200 bg-white p-16">
            ...
            <div class="mt-16 grid gap-2">
                ...
                <p class="mt-4 text-xs text-gray-500">{{ t('wizard.layers.source.description') }}</p>
                <input
                    v-model="draft.url"
                    class="w-full rounded-lg border p-8 text-sm outline-none transition"
            ...
            <div class="mt-16 grid gap-4">
                ...
                <div v-if="draft.selectedType === 'esri-map-image'" class="mt-16">
                    ...
                    <div
                        ...
                        class="mt-3 rounded-lg border border-dashed border-gray-300 p-12 text-sm text-gray-500"
                    >
                    ...
                    <div v-else class="mt-12 grid gap-3 ce-md:grid-cols-2">
                        <label
                            v-for="option in draft.sublayerOptions"
                            :key="option.index"
                            class="flex font-normal items-start gap-3 rounded-lg border border-gray-200 bg-white p-12"
                        >
                            <input
                                v-model="option.selected"
                                type="checkbox"
                                class="mt-2 h-4 w-4 rounded border-gray-300"
                            />
            ...
            <div
                ...
                class="mt-16 rounded-lg border border-orange-200 bg-orange-50 p-8 text-sm text-orange-800"
            >
                {{ draft.errorMessage }}
            </div>
            ...
            <div class="mt-16 flex justify-end">
                <button
                    type="button"
                    class="inline-flex items-center rounded-lg bg-gray-900 px-16 py-8 text-sm font-medium text-white transition hover:bg-black"
                    @click="addLayer"
                >
        ...
        <div class="mt-16 rounded-xl border border-gray-200 bg-white p-16">
            ...
            <div v-if="!layers.length" class="mt-12 text-sm text-gray-500">
            ...
            >
                <template #item="{ element: l, index }">
                    <div class="rounded-lg border border-gray-200 bg-white">
                        <div class="flex items-center gap-12 p-12 shadow-sm">
                            <div
                                class="drag-handle flex cursor-grab active:cursor-grabbing items-center justify-center rounded-md border border-gray-200 bg-gray-50 p-8 text-sm text-gray-500"
                            ...
                            <div class="flex items-center gap-8">
                                <button
                                    type="button"
                                    class="rounded-lg border border-gray-300 p-8 text-sm text-gray-700 transition hover:bg-gray-50"
                        ...
                        <div v-if="expandedLayerId === l.id" class="border-t border-gray-200 bg-gray-50 p-16">
                            <div class="grid gap-4">
                                ...
                                <div class="grid gap-2">
                                    ...
                                    <div
                                        class="rounded-lg border border-gray-200 bg-white p-8 text-sm text-gray-600"
                                    >
                                        <span>{{ l.url }}</span>
                                    </div>
                                </div>
                                ...
                                <div
                                    class="rounded-xl border border-gray-200 bg-white p-12"
                                ...
                                >
                                    <div v-if="l.layerType === 'esri-feature'">
                                        ...
                                        <div
                                            ...
                                            class="mt-16 space-y-4 max-h-[420px] overflow-y-auto pr-4"
                                        >
                                            <div>
                                                <h5 class="text-sm font-medium text-gray-900">
                                                ...
                                                <div
                                                    ...
                                                    class="mt-12 rounded-lg border border-dashed border-gray-300 p-12 text-sm text-gray-500"
                                                >
                                                    {{ t('wizard.layers.advanced.metadata.none') }}
                                                </div>

                                                <!-- Edit field metadata options -->
                                                <div v-else class="mt-12 grid gap-2 ce-md:grid-cols-2 ce-lg:grid-cols-3">
                                                    <div
                                                        v-for="field in editDraft.fieldInfo"
                                                        :key="field.name"
                                                        class="rounded-lg border border-gray-200 bg-white p-12"
                                                    >
                                            ...
                                            <div>
                                                ...
                                                <div v-else class="mt-12 grid gap-2 ce-md:grid-cols-2 ce-lg:grid-cols-3">
                                                    ...
                                                    >
                                                        ...
                                                        <label class="font-normal block text-[11px] text-gray-500">
                                                        ...
                                                        <label
                                                            class="font-normal inline-flex items-center gap-2 mt-12 text-xs text-gray-700"
                                                        >
                                                            <input
                                ...
                                <div class="flex justify-end gap-8 mt-4">
                                    ...
                                    <button
                                        type="button"
                                        class="rounded-lg bg-gray-900 p-8 text-sm font-medium text-white transition hover:bg-black"
                                        @click="saveEditLayer"
                                    >
...
const layerTypeLabels: Record<string, string> = {
    'file-geojson': t('wizard.layers.types.geojson'),
    'file-shape': t('wizard.layers.types.shape'),
    'file-csv': t('wizard.layers.types.csv'),
    'esri-feature': t('wizard.layers.types.feature'),
    'esri-map-image': t('wizard.layers.types.mil'),
    'esri-tile': t('wizard.layers.types.tile'),
    'esri-imagery': t('wizard.layers.types.imagery'),
    'ogc-wms': t('wizard.layers.types.wms'),
    'ogc-wfs': t('wizard.layers.types.wfs')
};
...
watch(
    ...
    async (layerType: string) => {
        ...
        try {
            ...
            draft.errorMessage = t('wizard.layers.errors.sublayers.load');
        } finally {
            loadingSublayers.value = false;
        }
    }
...
const onUrlInput = () => {
    ...
    if (!valid.value) {
        draft.errorMessage = t('wizard.layers.errors.url.invalid');
        return;
    }
...
const runDetection = () => {
    ...
    if (!possible.length && draft.url.trim()) {
        draft.errorMessage = t('wizard.layers.errors.url.undetect');
    }
};
...
const addLayer = async () => {
    ...
    if (!draft.url.trim()) {
        draft.errorMessage = t('wizard.layers.errors.url.required');
        return;
    }
    ...
    if (!draft.name.trim()) {
        draft.errorMessage = t('wizard.layers.errors.name.required');
        return;
    }

    if (!draft.selectedType) {
        draft.errorMessage = t('wizard.layers.errors.type.required');
        return;
    }
    ...
    if (draft.selectedType === 'esri-map-image' && !selectedSublayers(draft.sublayerOptions).length) {
        draft.errorMessage = t('wizard.layers.errors.sublayer.required');
        return;
    }
    ...
    try {
        ...
        draft.errorMessage = t('wizard.layers.errors.metadata.load');
        return;
    } finally {
