...
const addLayer = async () => {
    ...
    // fetch fields for feature layers and sublayers for MIL layers
    try {
        ...
        } else if (draft.selectedType === 'esri-map-image') {
            const sublayers = await fetchMapImageLayers(trimmedUrl);
            sublayerOptions = buildMapImageSublayerOptions(sublayers);
        }
    } catch (error) {
