<template>
    <div class="rounded-xl border border-gray-200 bg-white w-1/2">
        ...
        <div v-if="!layers.length" class="px-4 py-6 text-sm text-gray-500">
        ...
        <div v-else>
            ...
            <div class="relative legend-header flex items-center">
                <!-- add layer -->
                <div class="p-4 text-gray-500 hover:text-black">
                    <svg class="h-[18px] w-[18px] fill-current scale-x-[-1]" viewBox="0 0 24 24">
                        <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"></path>
                    </svg>
                </div>
                <!-- layer reorder -->
                <div class="p-2 text-gray-500 hover:text-black">
                    <svg class="h-[18px] w-[18px] fill-current scale-x-[-1]" viewBox="0 0 24 24">
            ...
            <div class="divide-y divide-gray-200">
                <div v-for="(layer, idx) in layers" :key="layer.id" class="relative">
                    <div class="flex items-center hover:bg-gray-200 default-focus-style p-3 transition">
                        <!-- symbology / icon block -->
                        <div
                            class="mr-3 flex h-8 w-8 shrink-0 items-center justify-center rounded bg-gray-100"
                            :style="{ color: getLayerColor(layer, idx) }"
                        >
                            <template v-if="layer.layerType === 'esri-feature'">
                                <div
                                    class="h-3 w-3 rounded-full border border-black bg-current"
                                    style="border-color: currentColor"
                                ></div>
                            </template>

                            <template v-else-if="layer.layerType === 'esri-map-image'">
                                <div class="grid h-4 w-4 grid-cols-2 gap-[2px]">
                                    <div
                            ...
                            <template v-else>
                                <div class="h-3 w-3 rounded-sm bg-current" style="border-color: currentColor"></div>
                            </template>
                        </div>
